...
class WeiboSpider(Spider):
    ...
    def start_requests(self):
        ...
        date2 = date1[1:]
        date1 = date1[:-1]
        # 由于意外断开之后，断点续存的方法 date3 date4
        date3 = []
        for i in pd.date_range('10/25/2018', '12/31/2018'):
            date3.append(str(i).replace("-", '').split()[0])
        date4 = date3[1:]
        date3 = date3[:-1]
        # 上面是哪里日期断开，修改的部分
        ...
        for uid in start_uids:
            uid = uid.strip()
            if uid == '科技':    # 断开的关键词
                for i, j in zip(date3, date4):
                    yield Request(
                        url="https://weibo.cn/search/mblog?hideSearchFrame=&keyword={}&advancedfilter=1&starttime={}&endtime={}&sort=hot&page=1".format(
                            uid, i, j), callback=self.parse_tweet)
            else:
                for i, j in zip(date1, date2):
                # for i, j in zip(range(20180101, 20180131), range(20180102, 20180132)):
                    yield Request(url="https://weibo.cn/search/mblog?hideSearchFrame=&keyword={}&advancedfilter=1&starttime={}&endtime={}&sort=hot&page=1".format(uid, i, j), callback=self.parse_tweet)
    ...
    def parse_tweet(self, response):
        ...
        for tweet_node in tweet_nodes:
            try:
                ...
                # 自己新增找关键词
                keyword = re.search(r'keyword=.*?&', response.url)
                keyword = keyword.group(0).replace('keyword=', '').replace('&', '')
                keyword = urllib.parse.unquote(keyword)
                tweet_item['keyword'] = keyword
                # tweet_item['crawl_time'] = int(time.time())
                tweet_item['crawl_time'] = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                ...
                comment_num_1 = int(re.search('\d+', comment_num).group())
                ...
                # 这里增加一个判断，把超过1W评论的微博单独提取出来做单独分析
                if comment_num_1 < 10001:
                    yield Request(url=comment_url, callback=self.parse_comment, meta={'weibo_url': tweet_item['weibo_url']})
                else:
                    f2 = open("comment_url.txt", "a+")
                    f2.write(tweet_item['weibo_url'] + ' ' + comment_url + '\n')
                    f2.close()
    ...
    def parse_comment(self, response):
        ...
        for comment_node in comment_nodes:
            ...
            comment_item['crawl_time'] = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
